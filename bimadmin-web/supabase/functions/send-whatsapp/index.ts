...
import { checkRateLimit, checkMonthlyAllowance } from "../_shared/rateLimit.ts";
...
Deno.serve(async (req) => {
  try {
    ...
    const { allowed: withinAllowance } = await checkMonthlyAllowance(supabase, organizationId);
    if (!withinAllowance) {
      return new Response(
        JSON.stringify({ error: "This organization has used its monthly message allowance. Upgrade the plan from Billing to send more this month." }),
        { status: 200 }
      );
    }
