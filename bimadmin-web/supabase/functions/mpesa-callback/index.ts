...
// If they're not set, the payment still activates fine; the receipt is
...
/** Best-effort receipt email. Deliberately never lets an email problem
 * affect whether the subscription itself gets activated -- that already
 * happened by the time this runs. */
async function sendReceipt(supabase: ReturnType<typeof createClient>, payment: any, receiptNumber: string | undefined) {
  try {
    ...
    await sendPlainEmail(
      email,
      `Receipt: KES ${payment.amount_kes.toLocaleString()} for BimAdmin ${plan?.name ?? ""}`,
      `<p>Hi ${businessName || "there"},</p>
       <p>This confirms your M-Pesa payment of <strong>KES ${payment.amount_kes.toLocaleString()}</strong> for the ${plan?.name ?? ""} plan.</p>
       ${receiptNumber ? `<p>M-Pesa receipt number: <strong>${receiptNumber}</strong></p>` : ""}
       <p>Thank you for using BimAdmin.</p>`
    );
  } catch {
    // Swallow -- a failed receipt should never surface as a payment error.
  }
}
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (verifiedStatus === "success") {
      ...
      await sendReceipt(supabase, payment, receipt);
    } else {
