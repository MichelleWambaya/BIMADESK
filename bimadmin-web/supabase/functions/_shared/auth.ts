...
// organization, not just any logged-in BimAdmin user. Without this,
...
//
// Note on status codes across the functions that use this: they
// deliberately always return HTTP 200, even for auth failures, rate
// limits, and provider rejections, putting the real outcome in the JSON
// body's `error` field instead. This is not an oversight -- supabase-js's
// functions.invoke() only exposes a generic "Edge Function returned a
// non-2xx status code" message on FunctionsHttpError; it does not parse
// the response body into that error automatically. Returning 200 with
// `{ error: "..." }` is the only way for the specific, useful message
// this code writes to actually reach the browser.
