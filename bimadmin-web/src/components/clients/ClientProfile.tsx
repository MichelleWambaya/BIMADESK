...
export function ClientProfile() {
  ...
  const documents = store.documentsFor("client", client.id);
  ...
  return (
    <div className="space-y-4">
      ...
      {deleting && (
        <ConfirmDeleteDialog
          title="Delete client"
          confirmText={clientDisplayName(client)}
          summaryLines={[
            `${policies.length} polic${policies.length === 1 ? "y" : "ies"}`,
            `${quotations.length} quotation${quotations.length === 1 ? "" : "s"}`,
            `${allTasks.length} task${allTasks.length === 1 ? "" : "s"}`,
            `${comms.length} logged communication${comms.length === 1 ? "" : "s"}`,
            `${notes.length} note${notes.length === 1 ? "" : "s"}`,
            `${documents.length} document${documents.length === 1 ? "" : "s"}`,
          ]}
          onClose={() => setDeleting(false)}
          onConfirm={async () => {
            const { error } = await store.deleteClient(client.id);
            if (!error) navigate("/app/clients", { replace: true });
            return error;
          }}
        />
      )}
