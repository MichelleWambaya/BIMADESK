...
import { KeyRound, ShieldCheck, LogOut, AlertTriangle, Camera } from "lucide-react";
      ...
      <div className="wb-card p-5 flex items-center gap-4">
        <div className="relative shrink-0">
          {profile?.avatarUrl ? (
            ...
            <div
              className="w-14 h-14 rounded-full flex items-center justify-center text-white text-[22px] font-display"
          ...
          <label
            className="absolute -bottom-1 -right-1 w-7 h-7 rounded-full bg-paper-raised border border-line flex items-center justify-center cursor-pointer hover:bg-paper-sunk"
            title="Change picture"
          >
            {uploadingAvatar ? (
              <span className="w-3.5 h-3.5 border-2 border-violet-200 border-t-violet-500 rounded-full animate-spin" />
            ) : (
              <Camera size={13} className="text-ink-soft" />
            )}
            <input
              type="file"
              accept="image/*"
        ...
        <div className="min-w-0">
          ...
          {profile?.avatarUrl && (
            <button
              className="text-[11.5px] text-ink-faint underline mt-1 hover:text-coral-500"
              onClick={async () => {
                setAvatarError(null);
          ...
          {avatarError && <p className="text-[11.5px] text-coral-500 mt-1">{avatarError}</p>}
        </div>
      </div>
      ...
      <div className="wb-card divide-y divide-line">
        ...
        <button className="w-full flex items-center gap-3 px-5 py-3.5 hover:bg-paper-sunk text-left" onClick={handleSignOut}>
          ...
          <div className="flex-1">
            ...
            <p className="text-[11.5px] text-ink-faint">Sign out of BimAdmin on this device</p>
          </div>
        </button>
      </div>
