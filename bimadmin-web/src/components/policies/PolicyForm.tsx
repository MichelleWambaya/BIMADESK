...
export function PolicyForm({ presetClientId, onDone }: { presetClientId?: string; onDone: (policyId: string) => void }) {
  const store = useApp();
  const { policyLimitReached, effectivePlan } = useSubscription();
  ...
  // Checked before rendering the form rather than on submit, so nobody
  // fills in eight fields only to be told at the end that they cannot
  // save it.
  if (policyLimitReached) {
    return (
      <UpgradePrompt
        feature="More policies"
        description={`You've used all ${effectivePlan?.maxPolicies ?? 0} policies included in the ${effectivePlan?.name ?? "current"} plan. Upgrading lifts the limit, and your existing policies keep working either way.`}
      />
    );
  }
  ...
  async function submit(e: React.FormEvent) {
    ...
    setSubmitting(false);
    if (!policy) {
      // The database enforces the cap too, so this can fire if a teammate
      // used the last slot while this form was open.
      const message = store.lastError ?? "";
      if (message.includes("POLICY_LIMIT_REACHED")) {
        return setError(
          `You've reached the policy limit on the ${effectivePlan?.name ?? "current"} plan. Upgrade from Billing to add more.`
        );
      }
      return setError("Could not save the policy. Please try again.");
    }
    onDone(policy.id);
  }
