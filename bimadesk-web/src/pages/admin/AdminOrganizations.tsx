  ...
  return (
    <div className="space-y-5">
      <div>
        ...
        <p className="text-black/50 text-[13px]">Every business using BimaDesk, and their current plan.</p>
      </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-[14px] overflow-hidden">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-white/5 last:border-0">
                ...
                <td className="px-4 py-2.5 text-black/70">{r.clientCount}</td>
                <td className="px-4 py-2.5 text-black/70">{r.planName}</td>
        ...
        {!loading && rows.length === 0 && <p className="text-black/40 text-[13px] p-6 text-center">No organizations yet.</p>}
      </div>
    </div>
  );
}
