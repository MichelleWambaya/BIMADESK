    ...
    <div className="space-y-5">
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-black/50 text-[13px]">Every M-Pesa and card payment across the platform.</p>
        </div>
      ...
      <div className="bg-white/5 border border-white/10 rounded-[14px] overflow-hidden">
        <table className="w-full text-[13px]">
          <thead>
            <tr className="border-b border-white/10 text-black/50 text-left text-[11.5px]">
          ...
          <tbody>
            {rows.map((r) => (
              <tr key={r.id} className="border-b border-white/5 last:border-0">
                ...
                <td className="px-4 py-2.5 text-black/70">KES {r.amountKes.toLocaleString()}</td>
                <td className="px-4 py-2.5 text-black/70 capitalize">{r.provider === "mpesa" ? "M-Pesa" : "Card"}</td>
                ...
                <td className="px-4 py-2.5 text-black/50">{formatDateTime(r.createdAt)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        {!loading && rows.length === 0 && <p className="text-black/40 text-[13px] p-6 text-center">No payments yet.</p>}
      </div>
    </div>
  );
}
