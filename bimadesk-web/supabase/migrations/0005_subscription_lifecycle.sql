...
begin
  ...
  loop
    if not exists (
      ...
      values (sub.organization_id, 'renewal_approaching', 'Your BimaDesk subscription renews in the next few days. Pay from Billing to keep your current plan.');
    end if;
  end loop;
end;
$$;
...
begin
  ...
  loop
    ...
    insert into notifications (organization_id, type, message)
    values (sub.organization_id, 'renewal_approaching', 'Your BimaDesk subscription payment did not go through. Pay within a few days to avoid losing access to paid features.');
  end loop;
  ...
  loop
    ...
    insert into notifications (organization_id, type, message)
    values (sub.organization_id, 'renewal_approaching', 'Your BimaDesk subscription moved to the Free plan after payment did not go through. Upgrade any time from Billing.');
  end loop;
end;
$$;
